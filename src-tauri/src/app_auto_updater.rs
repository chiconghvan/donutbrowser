...
impl AppAutoUpdater {
  ...
  pub async fn check_for_updates(
    ...
    if self.should_update(&current_version, &latest_release.tag_name, is_nightly) {
      ...
      let release_page_url = format!(
        "https://github.com/chiconghvan/donutbrowser/releases/tag/{}",
        latest_release.tag_name
      );
  ...
  async fn fetch_app_releases(
    ...
    let url = "https://api.github.com/repos/chiconghvan/donutbrowser/releases?per_page=100";
    let response = self
      .client
      .get(url)
...
"#,
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      if let Some(installer_path) = pending {
        ...
        let current_pid = std::process::id();
        let temp_dir = std::env::temp_dir();
        let script_path = temp_dir.join("donut_update_restart.bat");
        ...
        let install_command = match ext.as_str() {
          "exe" => format!(
            "start \"\" /wait \"{}\" /S /UPDATE",
            installer_path.to_str().unwrap()
          ),
          "msi" => {
            ...
            format!(
              "start \"\" /wait \"{}\" /i \"{}\" /quiet /norestart /promptrestart",
              msiexec,
              installer_path.to_str().unwrap()
            )
          }
