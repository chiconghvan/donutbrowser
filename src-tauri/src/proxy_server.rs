...
async fn handle_http_via_socks4(
  ...
  let target_ip = match target_host.parse::<std::net::Ipv4Addr>() {
    Ok(ip) => ip,
    Err(_) => match tokio::net::lookup_host((target_host, target_port)).await {
      Ok(mut addrs) => match addrs.find_map(|addr| match addr.ip() {
        std::net::IpAddr::V4(ip) => Some(ip),
        std::net::IpAddr::V6(_) => None,
      }) {
        Some(ip) => ip,
        None => {
          let mut response = Response::new(Full::new(Bytes::from("No IPv4 address found")));
          *response.status_mut() = StatusCode::BAD_GATEWAY;
          return Ok(response);
        }
      },
  ...
  socks_request.extend_from_slice(&target_ip.octets());
